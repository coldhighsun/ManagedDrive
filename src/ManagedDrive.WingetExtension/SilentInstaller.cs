...
internal static class SilentInstaller
{
    ...
    public static bool TryInstall(IReadOnlyList<string> packageSelectorArgs, bool useFullSilent, out int exitCode)
    {
        ...
        try
        {
            Console.WriteLine($"wingetex: downloading {string.Join(' ', packageSelectorArgs)} installer...");
            ...
            {
                Console.Error.WriteLine("wingetex: winget download reported success but produced no manifest file; falling back to `winget install`.");
                exitCode = 0;
                return false;
            }
            ...
            {
                Console.WriteLine("wingetex: running installer (this may take a moment)...");
        ...
        {
            Console.Error.WriteLine($"wingetex: {ex.Message}");
            exitCode = 1;
            return true;
        }
        finally
    ...
    {
        var userTempRoot = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Temp", "wingetex");
